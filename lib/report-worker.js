...
const { storeBuffer } = require("./file-storage");
...
async function processNext(pool) {
  ...
  try {
    ...
    await storeBuffer(pdf, checksum);
    ...
    try {
      ...
      const file = await client.query("insert into stored_files (unit_id,uploaded_by_user_id,file_type,storage_provider,storage_key,original_filename,mime_type,file_size_bytes,checksum) values ($1,$2,'report_pdf','render_disk',$3,$4,'application/pdf',$5,$3) returning *", [job.unit_id, job.requested_by_user_id, checksum, filename, pdf.length]);
