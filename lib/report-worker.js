...
async function processNext(pool) {
  ...
  const claimed = await pool.query(
    "update report_generation_jobs set status='processing',claim_token=$1,attempts=attempts+1,started_at=now(),lease_expires_at=now()+interval '2 minutes',error_message=null where id=(select id from report_generation_jobs where ((status='queued' and (lease_expires_at is null or lease_expires_at<now())) or (status='processing' and lease_expires_at<now())) and attempts<3 order by created_at,id for update skip locked limit 1) returning *",
    [token]
  );
