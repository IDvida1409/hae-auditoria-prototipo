...
const { storedPath } = require("./file-storage");
...
async function reportData(pool, job) {
  ...
  const audits = await pool.query(
    "select a.*,ar.name as area_name,c.label as cycle_label,u.full_name as auditor_name from audits a join audit_areas ar on ar.id=a.area_id join audit_cycles c on c.id=a.cycle_id left join app_users u on u.id=a.auditor_user_id where a.unit_id=$1 and a.status='finished' and ($2::uuid is null or a.area_id=$2) and c.month_start>=$3 and c.month_start<$4 order by c.month_start desc,ar.display_order,a.finished_at desc",
  ...
  for (const file of linked.rows) {
    ...
    const bytes = await fs.readFile(storedPath(file.storage_key));
    photos.push({ ...file, src: "data:" + file.mime_type + ";base64," + bytes.toString("base64") });
  }
...
async function processNext(pool) {
  ...
  try {
    ...
    await fs.mkdir(path.dirname(storedPath(checksum)), { recursive: true });
    await fs.writeFile(storedPath(checksum), pdf);
