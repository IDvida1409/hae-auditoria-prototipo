...
async function applyOperation(db, op) {
  ...
  if (op.entity_type === "audit" && op.operation === "create") {
    ...
    const audit = await db.query(
      "insert into audits (unit_id,area_id,checklist_id,cycle_id,auditor_user_id,device_id,local_audit_id,source,status,started_at,offline_created,sync_status) values ($1,$2,$3,$4,$5,$6,$7,'tablet_android','in_progress',$8,true,'synced') on conflict (device_id,local_audit_id) where device_id is not null and local_audit_id is not null do update set local_audit_id = excluded.local_audit_id returning *",
      [a.unit_id, a.id, a.checklist_id, cycle.rows[0].id, op.user_id, op.device_id, p.localAuditId, p.startedAt || op.created_at]
    );
  ...
  if (op.entity_type === "audit" && op.operation === "finalize") {
    ...
    // Plan generation is deferred until its workflow rules are defined.
    ...
    return { entityType: "audit", entityId: audit.id, audit: result.rows[0], actionPlanGeneration: "pending_workflow_definition" };
  }
