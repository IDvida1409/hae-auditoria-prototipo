...
const { weightedAuditScore } = require("./scoring");
...
async function applyOperation(db, op) {
  ...
  if (op.entity_type === "audit_answer") {
    ...
    const answer = await db.query(
      ...
      [audit.id, p.questionId, p.answer, p.answer === "C" ? Number(question.rows[0].weight) : p.answer === "NC" ? 0 : null, question.rows[0].risk_level, p.notes || null, p.answeredAt || op.created_at]
    );
  ...
  if (op.entity_type === "audit" && op.operation === "finalize") {
    ...
    const score = await weightedAuditScore(db, audit.id);
    const result = await db.query(
      "update audits set status='finished',finished_at=coalesce(finished_at,now()),final_score=$2,sync_status='synced',updated_at=now() where id=$1 returning *",
      [audit.id, score]
    );
    ...
    await activityLog.record(db, { unitId: audit.unit_id, actorUserId: op.user_id, entityType: "audit", entityId: audit.id,
      action: "audit.finalized", metadata: { score, source: "offline_sync", reportJobId: reportJob?.id || null, actionPlanCount: generated.actionPlans.length } });
  ...
  if (op.entity_type === "action_plan_feedback") {
    ...
    await activityLog.record(db, { unitId: plan.rows[0].unit_id, actorUserId: op.user_id, entityType: "action_plan", entityId: p.actionPlanId,
      action: "action_plan.feedback_submitted", metadata: { feedbackId: feedback.rows[0].id, source: "offline_sync" } });
