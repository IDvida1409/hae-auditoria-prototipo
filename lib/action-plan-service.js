...
async function generateForAudit(db, audit, options = {}) {
  ...
  await activityLog.record(db, {
    unitId: audit.unit_id,
    actorUserId: audit.auditor_user_id,
    entityType: "action_plan_document",
    entityId: document.id,
    action: "action_plan.generated",
    metadata: { auditId: audit.id, generationMode, itemCount: actionPlans.length, availableToResponsible: available }
  });
