...
async function ensureBootstrapAdmin(pool) {
  const username = String(process.env.PRIMARY_ADMIN_USERNAME || "david.souza").trim().toLowerCase();
  const password = String(process.env.PRIMARY_ADMIN_PASSWORD || "12345678");
  if (!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(username)) throw new Error("PRIMARY_ADMIN_USERNAME inválido.");
  validPassword(password);
  ...
  if (reusable.rows[0]) {
    await pool.query("update app_users set full_name='David Souza',email='david.souza@idauditor.local',username=$2,password_hash=$3,role='admin',must_change_password=false,active=true,platform_scope='unit',updated_at=now() where id=$1", [reusable.rows[0].id, username, encoded]);
  ...
  const inserted = await pool.query(`insert into app_users (unit_id,full_name,email,password_hash,role,platform_scope,username,must_change_password,active)
    values ($1,'David Souza','david.souza@idauditor.local',$2,'admin','unit',$3,false,true) returning id`, [unit.rows[0].id, encoded, username]);
  return inserted.rows[0].id;
}
async function ensureDemoResponsible(pool) {
  const source = await pool.query("select unit_id,password_hash from app_users where lower(username)=$1 and active=true limit 1", [String(process.env.PRIMARY_ADMIN_USERNAME || "david.souza").toLowerCase()]);
  if (!source.rows[0]?.password_hash) return null;
  const existing = await pool.query("select id,unit_id from app_users where lower(username)='carlos.01' limit 1");
  let userId = existing.rows[0]?.id;
  let unitId = existing.rows[0]?.unit_id;
  if (!userId) {
    const inserted = await pool.query(`insert into app_users (unit_id,full_name,email,password_hash,role,platform_scope,username,must_change_password,active)
      values ($1,'Carlos Lima','carlos.01@idauditor.local',$2,'area_responsible','assigned_areas','carlos.01',false,true) returning id`,
      [source.rows[0].unit_id, source.rows[0].password_hash]);
    userId = inserted.rows[0].id;
    unitId = source.rows[0].unit_id;
  } else {
    unitId = source.rows[0].unit_id;
    await pool.query(`update app_users set unit_id=$2,full_name='Carlos Lima',password_hash=$3,role='area_responsible',platform_scope='assigned_areas',must_change_password=false,active=true,updated_at=now() where id=$1`, [userId, unitId, source.rows[0].password_hash]);
  }
  await pool.query(`insert into user_area_permissions (user_id,unit_id,area_id,can_view_area_dashboard,can_view_area_scores,can_view_reports,can_submit_action_plan_feedback,can_view_feedback_history,can_view_notifications,can_justify_delay,active)
    select $1,a.unit_id,a.id,true,true,true,true,true,true,true,true from audit_areas a
    where a.slug='area-residuos' and a.unit_id=$2 and a.active=true
    on conflict (user_id,area_id) do update set active=true,can_view_area_dashboard=true,can_view_area_scores=true,can_view_reports=true,can_submit_action_plan_feedback=true,can_view_feedback_history=true,can_view_notifications=true,can_justify_delay=true,updated_at=now()`, [userId, unitId]);
  await pool.query("update audit_areas set responsible_user_id=$1 where slug='area-residuos' and unit_id=$2 and active=true", [userId, unitId]);
  return userId;
}
async function withAccessAreas(pool, user) {
  if (!user) return user;
  if (["admin", "quality", "auditor"].includes(user.role)) return { ...user, area_ids: [], area_slugs: [], all_areas: true };
  const areas = await pool.query(`select a.id,a.slug from user_area_permissions p join audit_areas a on a.id=p.area_id
    where p.user_id=$1 and p.unit_id=$2 and p.active=true and a.active=true order by a.display_order,a.name`, [user.id, user.unit_id]);
  return { ...user, area_ids: areas.rows.map((area) => area.id), area_slugs: areas.rows.map((area) => area.slug), all_areas: false };
}
...
async function handle(request, response, url, context) {
  ...
  try {
    ...
    if (url.pathname === "/api/access/login" && request.method === "POST") {
      ...
      if (!/^[a-z0-9][a-z0-9._-]{2,63}$/.test(username)) { sendJson(response, 400, { error: "Informe um usuário válido, como nome.sobrenome." }); return true; }
      ...
      sendJson(response, 200, { user: await withAccessAreas(pool, user), branding: await branding(pool, user) }); return true;
    }
    ...
    if (url.pathname === "/api/access/me" && request.method === "GET") { sendJson(response, 200, { user: await withAccessAreas(pool, user), branding: await branding(pool, user) }); return true; }
    ...
    if (url.pathname === "/api/access/users" && request.method === "POST") {
      ...
      try {
        ...
        if (body.areaId && scope === "assigned_areas") {
          ...
          if (body.role === "area_responsible") {
            await client.query("update audit_areas set responsible_user_id=$1,updated_at=now() where id=$2 and unit_id=$3", [result.rows[0].id, body.areaId, user.unit_id]);
          }
        }
        await client.query("commit");
...
module.exports = { handle, hashPassword, verifyPassword, validPassword, authenticated, temporaryCode, ensureBootstrapAdmin, ensureDemoResponsible, withAccessAreas };
