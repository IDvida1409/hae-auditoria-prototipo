...
async function ensureDemoResponsible(pool) {
  const source = await pool.query("select unit_id,password_hash from app_users where lower(username)=$1 and active=true limit 1", [String(process.env.BOOTSTRAP_ADMIN_USERNAME || "teste.01").toLowerCase()]);
