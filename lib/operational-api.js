...
async function handle(request, response, url, context) {
  ...
  const routes = new Set(["/api/offline-bootstrap", "/api/users", "/api/notifications", "/api/dashboard", "/api/reports", "/api/reports/register-client-pdf"]);
  ...
  try {
    ...
    if (url.pathname === "/api/reports/register-client-pdf" && request.method === "POST") {
      if (!["admin", "quality", "auditor"].includes(user.role)) {
        sendJson(response, 403, { error: "Perfil sem permissão para arquivar relatórios." });
        return true;
      }
      const body = await readJsonBody(request);
      if (!body.auditId || !body.fileId || body.reportType !== "monthly") throw new Error("Auditoria, arquivo e tipo de relatório são obrigatórios.");
      const client = await pool.connect();
      try {
        await client.query("begin");
        const auditResult = await client.query(
          `select a.*,c.label as cycle_label from audits a join audit_cycles c on c.id=a.cycle_id
           where a.id=$1 and a.unit_id=$2 and a.status='finished' and ($3::boolean or a.area_id=any($4::uuid[])) for update`,
          [body.auditId, unitId, allAreas, areaIds]
        );
        const audit = auditResult.rows[0];
        if (!audit) throw new Error("Auditoria finalizada não encontrada.");
        const fileResult = await client.query(
          "select * from stored_files where id=$1 and unit_id=$2 and uploaded_by_user_id=$3 and file_type='report_pdf' and mime_type='application/pdf'",
          [body.fileId, unitId, user.id]
        );
        const file = fileResult.rows[0];
        if (!file) throw new Error("Arquivo PDF não encontrado.");
        const title = "Relatório mensal - " + audit.cycle_label;
        const reportResult = await client.query(
          `insert into reports (unit_id,area_id,audit_id,cycle_id,generated_by_user_id,pdf_file_id,report_type,title,period_label,file_url,file_name,download_name,available_from)
           values ($1,$2,$3,$4,$5,$6,'monthly',$7,$8,$9,$10,$10,current_date)
           on conflict (area_id,cycle_id,report_type) where area_id is not null and cycle_id is not null
           do update set audit_id=excluded.audit_id,generated_by_user_id=excluded.generated_by_user_id,pdf_file_id=excluded.pdf_file_id,
             title=excluded.title,period_label=excluded.period_label,file_url=excluded.file_url,file_name=excluded.file_name,
             download_name=excluded.download_name,status='generated',available_from=current_date
           returning *`,
          [unitId, audit.area_id, audit.id, audit.cycle_id, user.id, file.id, title, audit.cycle_label, "/api/files/" + file.id + "/content", file.original_filename]
        );
        const report = reportResult.rows[0];
        let jobResult = await client.query(
          "select * from report_generation_jobs where unit_id=$1 and area_id=$2 and cycle_id=$3 and report_type='monthly' order by created_at desc limit 1 for update",
          [unitId, audit.area_id, audit.cycle_id]
        );
        if (!jobResult.rows[0]) {
          jobResult = await client.query(
            `insert into report_generation_jobs (unit_id,area_id,cycle_id,requested_by_user_id,report_type,status,payload,report_id,finished_at)
             values ($1,$2,$3,$4,'monthly','completed',$5,$6,now()) returning *`,
            [unitId, audit.area_id, audit.cycle_id, user.id, JSON.stringify({ auditId: audit.id, source: "approved_client_template" }), report.id]
          );
        } else {
          jobResult = await client.query(
            "update report_generation_jobs set status='completed',report_id=$2,error_message=null,finished_at=now(),lease_expires_at=null where id=$1 returning *",
            [jobResult.rows[0].id, report.id]
          );
        }
        await client.query(
          `insert into report_artifacts (report_id,file_id,job_id) values ($1,$2,$3)
           on conflict (job_id) do update set report_id=excluded.report_id,file_id=excluded.file_id,created_at=now()`,
          [report.id, file.id, jobResult.rows[0].id]
        );
        await client.query("commit");
        sendJson(response, 201, { report, storage: "postgres" });
      } catch (error) {
    ...
    if (auditDetail && request.method === "DELETE") {
      if (user.role !== "admin") {
        sendJson(response, 403, { error: "Somente o administrador pode excluir uma auditoria de teste." });
        return true;
      }
      const client = await pool.connect();
      try {
        await client.query("begin");
        const auditResult = await client.query(
          "select * from audits where id=$1 and unit_id=$2 for update",
          [auditDetail[1], unitId]
        );
        const audit = auditResult.rows[0];
        if (!audit) {
          await client.query("rollback");
          sendJson(response, 404, { error: "Auditoria não encontrada." });
          return true;
        }
        const [answers, plans, documents, reports] = await Promise.all([
          client.query("select id from audit_answers where audit_id=$1", [audit.id]),
          client.query("select id from action_plans where origin_audit_id=$1", [audit.id]),
          client.query("select id from action_plan_documents where origin_audit_id=$1", [audit.id]),
          client.query("select id,pdf_file_id from reports where audit_id=$1", [audit.id])
        ]);
        ...
        const linkedEntityIds = [audit.id, ...answerIds, ...planIds, ...documentIds, ...reportIds];
        const linkedFiles = linkedEntityIds.length
          ? await client.query("select distinct file_id from file_links where entity_id=any($1::uuid[])", [linkedEntityIds])
          : { rows: [] };
        const feedbackFiles = planIds.length
          ? await client.query(
              `select unnest(array_remove(array[evidence_file_id,response_file_id] || response_file_ids,null)) as file_id
               from action_plan_feedback where action_plan_id=any($1::uuid[])`,
              [planIds]
            )
          : { rows: [] };
        const artifactFiles = reportIds.length
          ? await client.query("select file_id from report_artifacts where report_id=any($1::uuid[])", [reportIds])
          : { rows: [] };
        const fileIds = [...new Set([
          ...linkedFiles.rows.map((row) => row.file_id),
          ...feedbackFiles.rows.map((row) => row.file_id),
          ...artifactFiles.rows.map((row) => row.file_id),
          ...reports.rows.map((row) => row.pdf_file_id)
        ].filter(Boolean))];
        const notificationIds = [...planIds, ...documentIds, ...reportIds];
        if (notificationIds.length) await client.query("delete from notifications where entity_id=any($1::uuid[])", [notificationIds]);
        if (linkedEntityIds.length) await client.query("delete from file_links where entity_id=any($1::uuid[])", [linkedEntityIds]);
        if (planIds.length) await client.query("delete from action_plans where id=any($1::uuid[])", [planIds]);
        if (documentIds.length) await client.query("delete from action_plan_documents where id=any($1::uuid[])", [documentIds]);
        if (reportIds.length) await client.query("delete from reports where id=any($1::uuid[])", [reportIds]);
        await client.query("delete from report_generation_jobs where payload->>'auditId'=$1", [audit.id]);
        await client.query(
          `delete from sync_queue where result_entity_id=$1 or payload->>'auditId'=$2
             or (device_id=$3 and payload->>'localAuditId'=$4)`,
          [audit.id, audit.id, audit.device_id, audit.local_audit_id]
        );
        await client.query("delete from audits where id=$1", [audit.id]);
        if (fileIds.length) await client.query("delete from stored_files where id=any($1::uuid[])", [fileIds]);
        await client.query("commit");
        sendJson(response, 200, {
          deleted: true,
          auditId: audit.id,
          removed: { answers: answerIds.length, plans: planIds.length, documents: documentIds.length, reports: reportIds.length, files: fileIds.length }
        });
      } catch (error) {
        await client.query("rollback");
        throw error;
