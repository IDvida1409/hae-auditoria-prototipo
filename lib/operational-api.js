  ...
  try {
    ...
    if (url.pathname === "/api/offline-bootstrap" && request.method === "GET") {
      const [areas, checklists, blocks, questions, settings] = await Promise.all([
        pool.query("select * from audit_areas where unit_id=$1 and active=true and ($2::boolean or id=any($3::uuid[])) order by display_order", [unitId, allAreas, areaIds]),
    ...
    if (url.pathname === "/api/audits" && request.method === "GET") {
      const result = await pool.query("select * from audits where unit_id=$1 and ($2::uuid is null or area_id=$2) and ($3::boolean or area_id=any($4::uuid[])) order by created_at desc,id limit $5 offset $6", [unitId, url.searchParams.get("areaId") || null, allAreas, areaIds, limit, offset]);
