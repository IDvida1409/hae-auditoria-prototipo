...
async function handle(request, response, url, context) {
  ...
  try {
    ...
    if (url.pathname === "/api/reports" && request.method === "GET") {
      const result = await pool.query("select * from reports where unit_id=$1 order by created_at desc,id limit $2 offset $3", [unitId, limit, offset]);
      sendJson(response, 200, { reports: result.rows, limit, offset, storage: "postgres" });
      return true;
    }
    if (reportVersions && request.method === "GET") {
      const result = await pool.query("select a.*,f.original_filename,f.file_size_bytes from report_artifacts a join reports r on r.id=a.report_id join stored_files f on f.id=a.file_id where r.id=$1 and r.unit_id=$2 order by a.created_at desc,a.id", [reportVersions[1], unitId]);
    ...
    if (url.pathname === "/api/offline-bootstrap" && request.method === "GET") {
      const [areas, checklists, blocks, questions, settings] = await Promise.all([
        pool.query("select * from audit_areas where unit_id=$1 and active=true order by display_order", [unitId]),
        pool.query("select * from checklists where unit_id=$1 and is_active=true order by created_at", [unitId]),
        pool.query("select b.* from checklist_blocks b join checklists c on c.id=b.checklist_id where c.unit_id=$1 and c.is_active=true order by b.display_order", [unitId]),
        pool.query("select q.* from checklist_questions q join checklist_blocks b on b.id=q.block_id join checklists c on c.id=b.checklist_id where c.unit_id=$1 and c.is_active=true order by q.question_number", [unitId]),
        pool.query("select * from audit_workflow_settings where unit_id=$1 and active=true", [unitId])
      ]);
    ...
    if (url.pathname === "/api/audits" && request.method === "GET") {
      const result = await pool.query("select * from audits where unit_id=$1 and ($2::uuid is null or area_id=$2) order by created_at desc,id limit $3 offset $4", [unitId, url.searchParams.get("areaId") || null, limit, offset]);
      sendJson(response, 200, { audits: result.rows, limit, offset });
      return true;
    }
    if (auditDetail && request.method === "GET") {
      const audit = await pool.query("select * from audits where id=$1 and unit_id=$2", [auditDetail[1], unitId]);
    ...
    if (url.pathname === "/api/dashboard" && request.method === "GET") {
      ...
      const [scores, risks, plans] = await Promise.all([
        pool.query("select a.area_id,avg(a.final_score)::numeric(5,2) as score,count(*)::int as audits from audits a join audit_cycles c on c.id=a.cycle_id where a.unit_id=$1 and c.month_start=$2 and a.status='finished' group by a.area_id", [unitId, month]),
        pool.query("select aa.risk_level_snapshot as risk,count(*)::int as count from audit_answers aa join audits a on a.id=aa.audit_id join audit_cycles c on c.id=a.cycle_id where a.unit_id=$1 and c.month_start=$2 and aa.answer='NC' group by aa.risk_level_snapshot", [unitId, month]),
        pool.query("select status,count(*)::int as count from action_plans where unit_id=$1 group by status", [unitId])
      ]);
