  ...
  try {
    ...
    if (url.pathname === "/api/reports" && request.method === "GET") {
      const [result, jobs] = await Promise.all([
        pool.query("select * from reports where unit_id=$1 and ($2::boolean or area_id=any($3::uuid[])) order by created_at desc,id limit $4 offset $5", [unitId, allAreas, areaIds, limit, offset]),
        pool.query(`select distinct on (area_id,cycle_id,report_type)
                     id,area_id,cycle_id,report_type,status,error_message,attempts,created_at,updated_at
                    from report_generation_jobs
                   where unit_id=$1 and ($2::boolean or area_id=any($3::uuid[]))
                   order by area_id,cycle_id,report_type,created_at desc`, [unitId, allAreas, areaIds])
      ]);
      sendJson(response, 200, { reports: result.rows, jobs: jobs.rows, limit, offset, storage: "postgres" });
      return true;
    }
