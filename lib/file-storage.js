...
const photoFileTypes = new Set(["audit_photo", "action_plan_photo"]);
const rasterPhotoFormats = new Set(["jpeg", "png", "webp", "gif", "tiff", "heif"]);
...
async function normalizePhoto(contents, filename) {
  try {
    let source = contents;
    const brand = contents.length >= 12 && contents.subarray(4, 8).toString("ascii") === "ftyp"
      ? contents.subarray(8, 12).toString("ascii").toLowerCase()
      : "";
    if (["heic", "heix", "hevc", "hevx", "heim", "heis", "hevm", "hevs", "mif1", "msf1"].includes(brand)) {
      source = Buffer.from(await convertHeic({ buffer: contents, format: "JPEG", quality: 0.92 }));
    }
    const image = sharp(source, { failOn: "error" });
    const metadata = await image.metadata();
    if (!metadata.width || !metadata.height || !rasterPhotoFormats.has(metadata.format)) throw new Error("Imagem sem formato ou dimensões válidas");
    const normalized = await image
      .rotate()
      .flatten({ background: "#ffffff" })
      .jpeg({ quality: 90, mozjpeg: true })
      .toBuffer();
    const base = path.basename(filename).replace(/(?:\.(?:jpe?g|jpe|jfif|png|webp|gif|tiff?|avif|heic|heif))+$/i, "").slice(0, 250) || "evidencia";
    return { contents: normalized, filename: `${base}.jpg`, mimeType: "image/jpeg" };
  } catch {
    throw new Error("A evidência precisa ser um arquivo de foto válido");
  }
}
...
async function upload(pool, request, user, device, unitId) {
  ...
  const requestedMimeType = String(request.headers["content-type"] || "").split(";")[0].toLowerCase();
  ...
  const isPhoto = photoFileTypes.has(fileType);
  if (isPhoto) {
    if (requestedMimeType && requestedMimeType !== "application/octet-stream" && !requestedMimeType.startsWith("image/")) throw new Error("A evidência precisa ser um arquivo de foto válido");
  } else if (!mimeTypes.has(requestedMimeType)) {
  ...
  try {
    ...
    const uploadedContents = await fsp.readFile(temporary);
    let contents = uploadedContents;
    let storedFilename = filename;
    let mimeType = requestedMimeType;
    if (isPhoto) {
      const normalized = await normalizePhoto(uploadedContents, filename);
      contents = normalized.contents;
      storedFilename = normalized.filename;
    ...
    const checksum = crypto.createHash("sha256").update(contents).digest("hex");
    const storedSize = contents.length;
    const client = await pool.connect();
    try {
      await client.query("begin");
      ...
      const saved = await client.query(
        ...
        [unitId, user.id, device.id, localFileId, fileType, checksum, storedFilename.slice(0, 255), mimeType, storedSize]
      );
      ...
      await client.query(
        ...
        [unitId, user.id, saved.rows[0].id, JSON.stringify({ fileType, mimeType, size: storedSize, checksum, originalFilename: filename })]
      );
      await client.query("commit");
      return saved.rows[0];
...
module.exports = { upload, download, migrateReportFiles, normalizePhoto, storeBuffer, storedPath, maxFileBytes, detectedMimeType, validFileStructure };
