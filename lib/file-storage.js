  ...
  try {
    ...
    try {
      ...
      if (prior.rows[0]) {
        ...
        await client.query("insert into stored_file_contents (file_id,contents) values ($1,$2) on conflict (file_id) do nothing", [prior.rows[0].id, await fsp.readFile(temporary)]);
      ...
      await client.query("insert into stored_file_contents (file_id,contents) values ($1,$2)", [saved.rows[0].id, await fsp.readFile(temporary)]);
      await client.query("commit");
      return saved.rows[0];
