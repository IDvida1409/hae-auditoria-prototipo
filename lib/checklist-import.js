...
async function upsertChecklist(client, unitId, area, data) {
  ...
  for (const [blockIndex, block] of (data.blocks || []).entries()) {
    ...
    for (const question of (block.questions || [])) {
      expectedQuestions.push(String(question.number));
      await client.query(
    ...
    await client.query(
      "update checklist_questions set active=false,updated_at=now() where block_id=$1 and not (question_number=any($2::text[]))",
